...
class TestPatientForm(TestCase):
    ...
    def test_missing_date(self):
        form_data = {'first_name': 'John', 'last_name': 'Smith', 'record_number': 'a'}
        form = NewPatient(data=form_data)
        self.assertFalse(form.is_valid())

    def test_missing_first_name(self):
        form_data = {'last_name': 'Smith', 'birth_date': '418', 'record_number': 'a'}
        form = NewPatient(data=form_data)
        self.assertFalse(form.is_valid())

    def test_missing_last_name(self):
        form_data = {'first_name': 'John', 'birth_date': '418', 'record_number': 'a'}
        form = NewPatient(data=form_data)
        self.assertFalse(form.is_valid())
